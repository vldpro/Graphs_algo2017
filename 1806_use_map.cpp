...
class Graph {
	...
	public:
		...
		void shortest_path() {
			...
			map<uint32_t, set<uint32_t>> pqueue; 

			for ( uint32_t i = 0; i < _nodes_count; i++ ) {
				pqueue[ _nodes[i].shortest_path ].insert(i);
			}

			while( !pqueue.empty() ) {
				auto nd = pqueue.begin();

				while ( !nd-> second.empty() ) {
					uint32_t i = *(nd-> second.begin());
					pqueue[ nd-> first ].erase(i);
					...
					for ( auto it = _nodes[i].adjacents.begin(); it != _nodes[i].adjacents.end(); it++ ) {
						...
						if ( _nodes[it-> first].shortest_path > _nodes[i].shortest_path + it-> second ) {
							pqueue[ _nodes[it-> first].shortest_path ].erase( it-> first );
							_nodes[it-> first].shortest_path = _nodes[i].shortest_path + it-> second;
							_nodes[it-> first].parent = i;
							pqueue[ _nodes[it-> first].shortest_path ].insert(it-> first);
						}
					}
				}

				pqueue.erase(nd-> first);
			}
		}
